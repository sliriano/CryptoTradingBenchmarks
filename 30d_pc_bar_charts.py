"""
Shows the monthly percent change of a given trading account and btc, eth as benchmarks.
"""
import pandas
import matplotlib.pyplot as plt
import requests

month_start_balance = 12000

# get pnl data from FTX csv file
df = pandas.read_csv("month_pnl.csv")
df = df[::-1].reset_index(drop=True) # reverse dataframe
total_pnl = float(df['total'].iloc[0])
acct_gain = float(total_pnl / month_start_balance) * 100

# get benchmark data
btc = requests.get("https://api.coingecko.com/api/v3/coins/bitcoin").json()
btc = btc['market_data']["price_change_percentage_30d"]

eth = requests.get("https://api.coingecko.com/api/v3/coins/ethereum").json()
eth = eth['market_data']["price_change_percentage_30d"]
...
ax.set_title("30 Day Percent Change of Trading Account & Benchmarks")
